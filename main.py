...
class WallPage(Handler):
  ...
  def get(self):
    ...
    # [END query]
    # Create the posts html to print on the comments pages and keep en eye out for errors
    posts_html = ''
...
class PostWall(webapp2.RequestHandler):
  ...
  def post(self):
    ...
    # Prepare for errors during user input validation. There will be errors, users are hopeless
    error = ''
    # Check user name for html, add to error if html found, but allow to continue
    ok_to_post = True
    validated_author = clean_html(user_name)
    if validated_author != str(user_name):
      error += "Don't use html with user names. "
    # Check for empty user name, add to error if true and do not allow to continue
    if validated_author == '':
      error += 'No user name found. '
      ok_to_post = False
    # Check for empty user comment, add to error if true and do not allow to continue
    if user_comment == '':
      error += 'Please write a comment before submitting. '
      ok_to_post = False
    # Check for swear words. Totally not accepted!
    if check_profanity(user_comment) == True or check_profanity(validated_author) == True:
      error += 'No dang swear words allowed! '
      ok_to_post = False
    if ok_to_post == True:
      # Otherwise allow to continue with post to the database
      post.author = Author(name=validated_author)
